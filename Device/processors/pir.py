...
from cmds.command_destination_factory import CommandDestinationFactory
...
class PIRProcessor(BaseProcessor):
    ...
    def start(self):
        ...
        for light in self._device.Settings["light_devices"]:
            dest = CommandDestinationFactory.createInstance(self._device.CommandDestination)
            dest.init(Device(ID=light))
            dest.start()
            self._commandDestList.append(dest)
    ...
    def stop(self):
        ...
        for dest in self._commandDestList:
            dest.stop()
...
pir._signalSource.signalReceived(1)

try:
    input('PIR processor started, press any key to exit\r\n')
except:
    pass
